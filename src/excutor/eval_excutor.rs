impl Expression {
    ...
    pub fn eval_mut(self, env: &mut Environment, depth: usize) -> Result<Self, LmError> {
        ...
        loop {
            match self {
                ...
                // 二元运算（内存优化）
                Self::BinaryOp(ref operator, ref lhs, ref rhs) => {
                    return handle_binary(&operator, lhs.to_owned(), rhs.to_owned(), env, depth);
                }
                ...
                // 其他复杂类型
                Self::Slice(ref list, ref slice_params) => {
                    return handle_slice(list, slice_params.to_owned(), env, depth);
                }
                Self::Index(ref lhs, ref rhs) => {
                    return handle_index(lhs.to_owned(), rhs.to_owned(), env, depth);
                }
                // 执行应用
    ...
    /// 处理复杂表达式的递归求值
    fn eval_complex(self, env: &mut Environment, depth: usize) -> Result<Self, LmError> {
        match self {
            // 控制流表达式
    ...
    /// 执行
    fn eval_apply(self, env: &mut Environment, depth: usize) -> Result<Self, LmError> {
        // 函数应用
        match self {
